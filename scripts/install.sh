...
echo ""
echo "  ${GREEN}syncforge server${NC}      — Start the sync server"
echo "  ${GREEN}syncforge init${NC}        — Create a project"
echo "  ${GREEN}syncforge join <id>${NC}   — Join a project"
echo "  ${GREEN}syncforge update${NC}      — Check for updates"
echo ""
echo "  (these commands work after global install from anywhere)"
echo ""
if [ "$GLOBAL_INSTALL" = true ]; then
  echo "  ${YELLOW}If 'syncforge' is not recognized, restart your terminal.${NC}"
fi
echo ""
